...
class VideosController extends Controller {
	...
	public function sendMessageToContacts($user_id, $video_id) {
		...
		$monitor_ids = MonitorOrganization::where('organization_id', $organization_id)->pluck('monitor_id')->all();
		...
		$monitors = Monitor::whereIn('monitor_id', $monitor_ids)->get();
		$monitors_email = $monitors->pluck('monitor_email')->toArray();
		$monitors_number = $monitors->pluck('phone_number')->toArray();
		...
		// $contacts_email1 = ['o16.ghulamrasool@gmail.com', 'o16.talha@gmail.com', 'o16.ahsan@gmail.com'];
		//MAIL to CC Admins

		try {
			Mail::send([], [], function ($message) use ($contacts_email, $type, $video_id) {
				if ($type == 'panic') {
					$body = 'A new SafeWalk is in progress. View Here: https://controlhub.agentsafewalk.com/dashboard?type=' . $type . '&video_id=' . $video_id;
				} elseif ($type == 'lost_person') {
					$body = 'A new Identification upload is in progress. View Here: https://controlhub.agentsafewalk.com/dashboard?type=' . $type . '&video_id=' . $video_id;
				} else {
					$body = 'A new Report Tip has been submitted. View Here: https://controlhub.agentsafewalk.com/dashboard?type=' . $type . '&video_id=' . $video_id;
				}
				$message->to($contacts_email)->subject('AgentSafeWalk | Request')->setBody($body, 'text/html');
			});
		} catch (\Exception $e) {
		...
		//MAIL to CC Monitors
		try {
			Mail::send([], [], function ($message) use ($monitors_email, $type, $video_id) {
				if ($type == 'panic') {
					$body = 'A new SafeWalk is in progress. View Here: https://controlhub.agentsafewalk.com/monitor-hub/safewalk?type=' . $type . '&video_id=' . $video_id;
				} elseif ($type == 'lost_person') {
					$body = 'A new Identification upload is in progress. View Here: https://controlhub.agentsafewalk.com/monitor-hub/identification_uploads?type=' . $type . '&video_id=' . $video_id;
				} else {
					$body = 'A new Report Tip has been submitted. View Here: https://controlhub.agentsafewalk.com/monitor-hub/safewalk?type=' . $type . '&video_id=' . $video_id;
				}
				$message->to($monitors_email)->subject('AgentSafeWalk | Request')->setBody($body, 'text/html');
			});
		} catch (\Exception $e) {
			$exceptionmessage = $e->getMessage();
		}
		// Msg on phone to CC Admins
		if ($type == 'panic') {
			$body = 'A new SafeWalk is in progress. View Here: https://controlhub.agentsafewalk.com/dashboard?type=' . $type . '&video_id=' . $video_id;
		} elseif ($type == 'lost_person') {
			$body = 'A new Identification upload is in progress. View Here: https://controlhub.agentsafewalk.com/dashboard?type=' . $type . '&video_id=' . $video_id;
		...
		foreach ($monitors_number as $numberitem) {
		...
		if (apiHelper::authenticate($user_id, $auth_id)) {
			...
			try {
				Mail::send([], [], function ($message) use ($user_contacts_email, $user_first_name) {
					...
					$message->to($user_contacts_email)->subject('AgentSafeWalk | Your Friend Need Help')->setBody($body, 'text/html');
				});
			} catch (\Exception $e) {
